...
const fs = require("fs");
const shixd = require("./settings.json");

client.on('ready', async () => {
  client.user.setPresence({ activity: { name: "/başvuru"}, status: "idle"});
  let botVoiceChannel = client.channels.cache.get(shixd.ses);
...
fs.readdir("./Commands", (err, files) => {
  ...
  client.on("message", (message) => {
        if (message.author.bot ||!message.content.startsWith(shixd.prefix) || !message.channel || message.channel.type == "dm") return;
        let args = message.content
          .substring(shixd.prefix.length)
          .split(" ");
        let command = args[0];
...
client.login(shixd.token).then(console.log("Bot başarılı bir şekilde giriş yaptı.")).catch(e => console.error(e));
